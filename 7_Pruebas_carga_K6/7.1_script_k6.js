...
export function handleSummary(data) {
    ...
    for (const [name, metric] of Object.entries(data.metrics)) {
        ...
        if (metric.type === "counter" && v) {
            rows.push(`<tr><td>${name}</td><td>Counter</td><td>${safeVal(v.count, 0)}</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>`);
        }
        if (metric.type === "rate" && v) {
            rows.push(`<tr><td>${name}</td><td>Rate</td><td>-</td><td>-</td><td>-</td><td>-</td><td>${safeVal(v.rate * 100)}%</td></tr>`);
        }
        if (metric.type === "trend" && v) {
            rows.push(`<tr><td>${name}</td><td>Trend</td><td>${safeVal(v.min)}</td><td>${safeVal(v.avg)}</td><td>${safeVal(v.med)}</td><td>${safeVal(v["p(95)"])}</td><td>-</td></tr>`);
        }
    }

    const thresholds = [];
    for (const [, check] of Object.entries(data.metrics)) {
        if (check.thresholds && Array.isArray(check.thresholds) && check.thresholds.length) {
            for (const th of check.thresholds) {
                thresholds.push(`<tr><td>${th.name || check.name || "N/A"}</td><td>${th.source || "N/A"}</td><td>${th.ok ? "✅ Pass" : "❌ Fail"}</td></tr>`);
            }
        }
    }

    const checks = Object.entries(data.metrics)
        .filter(([name]) => name.startsWith("checks") && data.metrics[name]?.values)
        .map(([name, m]) => {
            const v = m.values;
            return `<tr><td>${name}</td><td>${safeVal(v.passes, 0)}</td><td>${safeVal(v.fails, 0)}</td><td>${safeVal(v.rate * 100)}%</td></tr>`;
        })
        .join("\n");
...
<div class="summary-box">
    ...
    <span>Duration:</span> ${safeVal(data.state?.testRunDurationMs, 0)} ms |
    <span>Total Requests:</span> ${totalReqs}
</div>
