...
- **Criterios de Aceptación:**
    ...
    - **Resiliencia de Caché:** Si la caché Redis está caída, la API debe registrar una advertencia (_warning_) y continuar consultando directamente al proveedor de mapas sin interrumpir el servicio.
- **Tareas Técnicas:**
    ...
    - Escribir pruebas simulando aciertos (_hit_) y fallos (_miss_) de caché.
...
**Para** obtener el listado detallado de puntos geográficos (path),

- **Criterios de Aceptación:**
    - **Caso Exitoso:** Al enviar `POST` a `/api/v1/geo/route` con origen y destino válidos, retorna un array `path` con los puntos del trayecto.
...
- **Criterios de Aceptación:**
    ...
    - **Retry Policy:** Ante una desconexión o fallo 5xx temporal del proveedor de mapas, el sistema reintenta la petición hasta 3 veces utilizando un retraso exponencial (_Exponential Backoff_) antes de reportar un error al cliente.
- **Tareas Técnicas:**
